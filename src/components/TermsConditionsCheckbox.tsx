  ...
  return (
    <div className="flex flex-col text-left input-wrapper">
      <label
        ...
        className="pr-4  mb-3 font-medium text-headerText"
        htmlFor={label}
      >
        {label}
      </label>
      <span className="inline-flex justify-left">
        <input
          id="checkbox"
          className="w-4 h-4 text-blue-600 border-gray-300 rounded bg-gray-200 focus:outline-none focus:border-sky-500 focus:ring-1 "
          type="checkbox"
          checked={value}
          name={name}
          onChange={onChange}
          disabled={disabled}
        />
        <label htmlFor={label} className="ml-2 text-sm font-medium text-headerText">
          I agree with the{' '}
